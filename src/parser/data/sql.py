...
class DataBase:
    ...
    def get_driver_profile(self, fio):
        with self.connection:
            self.cursor.execute('''
                    SELECT username_tg FROM drivers
                    WHERE fio = %s;
                ''', (fio,))
    ...
    def add_driver_to_database(self, fio, username, id):
        # Добавление водителя в таблицу
        with self.connection:
            self.cursor.execute('''
                INSERT INTO drivers (fio, username_tg, id_tg)
                VALUES (%s, %s, %s);
            ''', (fio, username, id)
            )
